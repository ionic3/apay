...
export class AidogPage {
	...
	DisableInvestment(currency){
		...
		title: 'Do you want to stop ai- trade?',
		message: 'Select Ok to stop ai-trade. Select cancel to continue',
		buttons: [
		{
		  text: 'Cancel',
		  handler: () => {
		...
		},
		{
		  	...
		  	handler: () => {
		        ...
		        .subscribe((data) => {
					...
					{
						this.AlertToast('Disable Ai-trade successfully','success_form');
	        	...
	        	else
	        	{
		        	...
		        	else
		        	{
				        ...
				        .subscribe((data) => {
							...
							if (data.status == 'complete')
							{
								this.AlertToast('Ai-trade successfully','success_form');
								this.status[currency] = 'on';
